...
SELECT nome, preco, estoque FROM produto WHERE preco > 500.00 ORDER BY preco DESC;
...
v.id AS id_venda,
 c.nome AS nome_cliente,
  v.data_venda,
   v.valor_total FROM venda AS v 
INNER JOIN cliente AS c ON v.id_cliente = c.id
WHERE v.valor_total > 1000.00 ORDER BY v.data_venda DESC;
